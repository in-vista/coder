...
options = $.extend({
    async: {
        saveUrl: `${window.dynamicItems.settings.wiserApiRoot}items/{itemIdEncrypted}/upload?propertyName=${encodeURIComponent("{propertyName}")}&itemLinkId={itemLinkId}&useTinyPng=${(options.useTinyPng === true).toString()}&useCloudFlare=${(options.useCloudFlare === true).toString()}&cloudFlareUseVariant=${options.cloudFlareUseVariant ?? 'original'}&entityType=${encodeURIComponent("{entityType}")}&linkType={linkType}`,
